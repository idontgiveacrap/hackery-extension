...
var LINTED_LANGUAGES = /* @__PURE__ */ new Set(["javascript", "json", "regex", "url"]);
...
function languageExtension(language2) {
  ...
  if (language2 === "url" || language2 === "plain" || language2 === "wildcard") {
    return [];
  }
  return javascript();
}
    ...
    return [
      {
        ...whole,
        severity: "warning",
        message: scheme[1].toLowerCase() === "javascript" ? "Bookmarklet, not a URL \u2014 set Type to Scriptlet." : `Only http(s) links open reliably; "${scheme[1]}:" may not.`
      }
    ];
  }
  return [];
...
function attachCodeMirror(element, options = {}) {
  ...
  const lintEnabled = options.lint ?? LINTED_LANGUAGES.has(language2);
